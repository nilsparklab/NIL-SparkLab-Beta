(function () {
  ...
  function sim() {
    try {
      const s = window.NILSparkLabLastSimulation || window.NilSparkLabSimulationState;
  ...
  function run(symptom) {
    ...
    render(result);
    try {
      window.dispatchEvent(new CustomEvent('nilsparklab:fault-copilot-complete', { detail: result }));
    } catch (_) {}
    return result;
  }
