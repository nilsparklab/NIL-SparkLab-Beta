...
(function(){
"use strict";
/* Study KB loader: data script must load before this runtime. */
var DATA={topics:[]};
try{
  if (window.NILStudyKB && typeof window.NILStudyKB === "object") {
    DATA=window.NILStudyKB;
  } else {
    console.warn("StudyKB data was not available when the Study runtime started.");
  }
}catch(e){
  console.warn("StudyKB load failed", e);
  DATA={topics:[]};
}
